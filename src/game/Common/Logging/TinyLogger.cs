...
namespace VolumetricStudios.VoxeliqGame.Common.Logging
{
    ...
    public class Logger
    {
        ...
        {
            LogRouter.RouteMessage(level, this.Name,
        ...
        {
            LogRouter.RouteException(level, this.Name,
                                     args == null ? message : string.Format(CultureInfo.InvariantCulture, message, args),
                                     exception);
        }
    ...
    public class FileTarget : LogTarget, IDisposable
    {
        ...
        public FileTarget(string fileName, Logger.Level minLevel, Logger.Level maxLevel, bool includeTimeStamps,
                          bool reset = false)
        {
        ...
        public override void LogMessage(Logger.Level level, string logger, string message)
        {
            lock (this) // we need this here until we seperate gs / moonet /raist
            {
                ...
                if (!this._disposed)
                    this._logStream.WriteLine(string.Format("{0}[{1}] [{2}]: {3}", timeStamp,
                                                            level.ToString().PadLeft(5), logger, message));
            }
        }
        ...
        public override void LogException(Logger.Level level, string logger, string message, Exception exception)
        {
            lock (this)
            {
                ...
                if (!this._disposed)
                    this._logStream.WriteLine(string.Format("{0}[{1}] [{2}]: {3} - [Exception] {4}", timeStamp,
    ...
    public class ConsoleTarget : LogTarget
    {
        ...
        public override void LogMessage(Logger.Level level, string logger, string message)
        {
            ...
            Console.WriteLine(string.Format("{0}[{1}] [{2}]: {3}", timeStamp, level.ToString().PadLeft(5), logger,
                                            message));
        }
        ...
        public override void LogException(Logger.Level level, string logger, string message, Exception exception)
        {
            ...
            SetForeGroundColor(level);
            Console.WriteLine(string.Format("{0}[{1}] [{2}]: {3} - [Exception] {4}", timeStamp,
                                            level.ToString().PadLeft(5), logger, message, exception));
        }
