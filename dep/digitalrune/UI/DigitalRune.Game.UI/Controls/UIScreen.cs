...
namespace DigitalRune.Game.UI.Controls
{
  ...
  /// <see cref="UIControl.X"/>, <see cref="UIControl.Y"/> like in a canvas, or using alignment 
  /// and margins as usual.
  /// </para>
  /// <para>
  ...
  /// does not limit the layout size of the child controls. - Children can be positions outside
  /// the screen area.
  /// </para>
  /// <para>
  /// Per default, screens are not focus scopes (see <see cref="FocusManager"/>) because usually 
  /// the 3D scene is drawn behind the screen and the screen should not "absorb" arrow key input 
  /// or thumbstick/direction pad input.
  /// </para>
  /// </remarks>
  public class UIScreen : UIControl, IDrawable
  {
    ...
    /// </summary>
    /// <value>
    /// The <see cref="FocusManager"/>. Can be set to another <see cref="FocusManager"/> but not
    ...
    /// </value>
    /// <remarks>
    /// Lower values mean the screen is drawn first, behind other screens. Screens are updated
    /// in the reverse order: The screen with the highest value is updated first because it covers 
    /// the other screens.
    /// </remarks>
    public int ZIndex
    {
 ...
 UIPropertyOptions.None);
    ...
    ///// derived class, be sure to call the base class’s <see cref="OnDrawOrderChanged"/> method so that 
    ...
    ///// derived class, be sure to call the base class’s <see cref="OnVisibleChanged"/> method so that 
    ///// registered delegates receive the event.
    ///// </remarks>
