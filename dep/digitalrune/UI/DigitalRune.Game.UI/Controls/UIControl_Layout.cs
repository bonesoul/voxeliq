...
namespace DigitalRune.Game.UI.Controls
{
  public partial class UIControl
  {
    ...
    /// </remarks>
    public object RendererInfo { get; set; }


    /// <summary>
    /// Gets a value indicating whether the cached <see cref="RendererInfo"/> is up-to-date.
    /// </summary>
    /// <value>
    /// <see langword="true"/> if the cached <see cref="RendererInfo"/> is up-to-date; otherwise, 
    /// <see langword="false"/>.
    /// </value>
    /// <remarks>
    ...
    /// <summary>
    /// Invalidates the cached <see cref="RendererInfo"/>.
    /// </summary>
    ...
    /// properties, it only returns the desired size as the result of the method. This method
    /// must call <see cref="Measure"/> of the visual children.
    /// </remarks>
    ...
    internal static void Arrange(UIControl control, Vector2F position, Vector2F constraintSize)
    {
      ...
      var childPosition = position + new Vector2F(control.X, control.Y);      
      
      Vector2F childSize = constraintSize;
      if (constraintSize.X > control.DesiredWidth)
      {
        // We have room to move the control horizontally. --> Check HorizontalAlignment.
        switch (control.HorizontalAlignment)
        {
          ...
          case HorizontalAlignment.Center:
            childPosition.X = childPosition.X + constraintSize.X / 2 - control.DesiredWidth / 2;
            childSize.X = control.DesiredWidth;
            break;
          case HorizontalAlignment.Right:
            childPosition.X = childPosition.X + constraintSize.X - control.DesiredWidth;
            childSize.X = control.DesiredWidth;
            break;
      ...
      if (constraintSize.Y > control.DesiredHeight)
      {
        // We have room to move the control vertically. --> Check VerticalAlignment.
        switch (control.VerticalAlignment)
        {
          ...
          case VerticalAlignment.Center:
            childPosition.Y = childPosition.Y + constraintSize.Y / 2 - control.DesiredHeight / 2;
            childSize.Y = control.DesiredHeight;
            break;
          case VerticalAlignment.Bottom:
            childPosition.Y = childPosition.Y + constraintSize.Y - control.DesiredHeight;
            childSize.Y = control.DesiredHeight;
            break;
