...
namespace DigitalRune.Game.UI.Controls
{
  ...
  /// </summary>
  /// <remarks>
  /// The text box can be a multi-line text box (if <see cref="MaxLines"/> is greater than 1).
  /// It supports vertical scrolling but not horizontal scrolling. On Windows text can be entered
  /// using the keyboard. On Windows Phone 7 the software keyboard is opened when the text box
  /// is clicked. On Xbox 360 the software keyboard is opened when the text box is focused and
  /// A on the gamepad is clicked.
  /// </remarks>
  public partial class TextBox : UIControl
  {
...
#if !WINDOWS
    // Create a clipboard replacement which is shared by all text boxes.
    ...
    // and not because of text wrapping. (When the number is positive, an \n must be inserted 
    // for text wrapping.)
    private List<int> _lineStarts;

    // True if the caret should be brought into the view. This flag is set when the CaretIndex is
    // changed.
    ...
    private int CaretIndex
    {
      ...
      set
      {
        ...
        // When the cursor is moved, the textbox should scroll to the cursor position.
    ...
    /// <see langword="false"/>. Only single-line text boxes (where <see cref="MaxLines"/> 
    /// is 1) can be password boxes.
    /// </value>
    ...
    /// This is a game object property.
    /// </summary>
    /// <value>The minimal number of desired lines.</value>
    public int MinLines
    {
    ...
    /// </summary>
    /// <value>
    /// The maximal number of lines. The default is 1. If this property is greater than 1, the
    /// text box is a multi-line text box (<see cref="IsMultiline"/>).
    /// </value>
    public int MaxLines
    {
    ...
    public TextBox()
    {
      ...
      // When IsReadOnly is changed, we must update the cursor because the I beam cursor should not 
      // be used over read-only text boxes.
    ...
    protected override void OnLoad()
    {
      ...
      // If the control is writeable, it uses an I beam cursor; otherwise, the default cursor.
      UpdateCursor();
      ...
      if (!string.IsNullOrEmpty(verticalScrollBarStyle))
      {
        ...
        // The I beam cursor should not be visible over the scroll bar. Therefore, the scroll
        // bar sets its desired cursor to the default cursor to override the I beam of the text box.
    ...
    protected override void OnHandleInput(InputContext context)
    {
      ...
      if (isMouseOver && !inputService.IsMouseOrTouchHandled)
      {
        ...
        else if (inputService.IsPressed(MouseButtons.Left, false))
        {
          ...
          inputService.IsMouseOrTouchHandled = true;          
          var index = GetIndex(context.MousePosition, screen);
        ...
        else
        {
          ...
          if (inputService.MouseWheelDelta != 0 && VerticalScrollBarVisibility != ScrollBarVisibility.Disabled)
          {
            ...
            float offset = VisualOffset - delta;
            if (offset < 0)
              offset = 0;
            else if (offset > _verticalScrollBar.Maximum)
              offset = _verticalScrollBar.Maximum;
    ...
    private void HandleKeyboardInput()
    {
      ...
      // Coerce caret index. (This can be necessary if the TextBox.Text property was changed
      ...
      // Other keys enter text if they have a readable character equivalent in the current key map.
    ...
    {
      var resultText = Guide.EndShowKeyboardInput(result);
      if (resultText != null)
        Text = resultText;
    }
#endif
