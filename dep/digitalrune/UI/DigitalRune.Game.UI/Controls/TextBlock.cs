...
namespace DigitalRune.Game.UI.Controls
{
  ...
  public class TextBlock : UIControl
  {
    ...
    /// </value>
    /// <remarks>
    /// If this value is <see langword="true"/>, the renderer should clip the text rendering. 
    /// This flag is set if text must be clipped within characters (e.g. if the whole text block
    /// is not high enough). The clipping rectangle is defined by the 
    /// <see cref="UIControl.ActualBounds"/> and applying the <see cref="UIControl.Padding"/>.
    /// </remarks>
    ...
    protected override Vector2F OnMeasure(Vector2F availableSize)
    {
      ...
      // Remove padding from constraintSize.
      Vector4F padding = Padding;
      var contentSize = availableSize;
      ...
      var size = (Vector2F)font.MeasureString(text);
      if (size < contentSize)
      {
      ...
      // Add last line.
      VisualText.Append(line.ToString());
