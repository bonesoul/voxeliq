...
namespace DigitalRune.Game.UI.Controls
{
  ...
  /// defined using the game object properties
  /// <see cref="RenderTransformOrigin"/>, <see cref="RenderTranslation"/>, 
  /// <see cref="RenderRotation"/> and <see cref="RenderScale"/>. The render transform can be used
  /// to scale, rotate and translate the control. The render transform is correctly applied when
  /// input is handled (e.g. mouse clicks). But the render transform is ignored in the layout 
  /// process.
  /// </para>
  /// <para>
  ...
  /// invalidate the layout if necessary. Derived classes can override <see cref="OnMeasure"/>
  /// and <see cref="OnArrange"/> to customize the layout results.
  /// </para>
  /// <para>
  ...
  public partial class UIControl : GameObject
  {
    ...
    // the children can remove themself from the original collection.
    ...
    {
      get { return RenderScale != Vector2F.One || RenderRotation != 0 || RenderTranslation != Vector2F.Zero; }
    }
    ...
    /// animations, but it should be used sparingly since it costs performance. When handling
    /// input (e.g. mouse clicks) the render transform is considered and, for example, rotated
    /// buttons can be clicked normally.
    /// </para>
    /// <para>
    ...
    /// children. Invisible controls are not drawn, they do not take part in the layout process
    /// and they do not handle input. See also <see cref="ActualIsVisible"/>.
    /// </remarks>
    ...
    /// </summary>
    /// <value>
    /// <see langword="true"/> if the mouse is over this control or over a visual child; 
    /// otherwise, <see langword="false"/>.
    /// </value>
    /// <remarks>
    /// <para>
    ...
    /// wrapped in a <see cref="TextBlock"/> and shown in the <see cref="ToolTipManager.ToolTipControl"/>.
    /// </description>
    /// </item>
    /// <item>
    /// <description>
    ...
    /// </para>
    /// <para>
    /// The user can override this behavior by setting a <see cref="ToolTipManager.CreateToolTipContent"/> 
    /// callback in the <see cref="ToolTipManager"/>. The callback receives the value stored in 
    ...
    internal void HandleInput(InputContext context)
    {
      ...
      // at the end.
      var originalAllowedPlayer = context.AllowedPlayer;
      var originalMousePosition = context.MousePosition;
      var originalMousePositionDelta = context.MousePositionDelta;
    ...
    /// <inheritdoc/>
    protected override void OnPropertyChanged<T>(GameProperty<T> property, T oldValue, T newValue)
    {
      // Check UIPropertyOptions and invalidate measure/arrange/render if needed.
      var options = GetPropertyOptions(property.Metadata.Id);
