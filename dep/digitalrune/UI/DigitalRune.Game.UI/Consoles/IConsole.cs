...
namespace DigitalRune.Game.UI.Consoles
{
  ...
  public interface IConsole
  {
    ...
    /// </summary>
    /// <remarks>
    /// If an event handler handles a command, it must set the <see cref="ConsoleCommandEventArgs.Handled"/>
    /// flag. Event handlers should check if this flag was already set by another handler. In some
    /// cases it make sense that more than one event handler handle a single command, for example:
    /// When "help" command was entered, all event handlers can output their help information.
    /// </remarks>
