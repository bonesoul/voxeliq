...
namespace DigitalRune.Game.UI.Rendering
{
  ...
  /// </para>
  /// <para>
  /// Note that a render transformation cannot be inversed. The inverse is not an SRT 
  ...
  public struct RenderTransform : IEquatable<RenderTransform>
  {
    ...
    {
      if (obj is RenderTransform)
        return this == ((RenderTransform)obj);
    ...
    {
      Vector2F location = new Vector2F(rectangle.X, rectangle.Y);
      Vector2F extent = new Vector2F(rectangle.Width, rectangle.Height);
      location = ToRenderPosition(location);
      extent = ToRenderDirection(extent);
      return new RectangleF(location.X, location.Y, extent.X, extent.Y + 0.5f);
    }
    ...
    {      
      ...
      var spriteExtent = sourceRectangle.HasValue 
      ...
      var screenExtent = spriteExtent * scale;
      ...
      var normalizedOrigin = relativeOrigin / screenExtent;

      // Now we can compute the trasform origin in the local space of the sprite.
      var spriteOrigin = normalizedOrigin * spriteExtent;     
