...
namespace DigitalRune.Game.UI.Rendering
{
  public partial class UIRenderer
  {
    ...
    // TODO: We could cache the caret animation info in UIControl.RendererInfo.
    private const float CaretBlinkTime = 0.4f;
    private const string StateId = "State";
    private readonly Stopwatch _caretTimer = Stopwatch.StartNew();
    ...
    // TODO: Since UIRenderer has a Texture2D - it should implement IDisposable, according to FxCop.
    ...
    /// <see cref="UIRenderContext.RenderTransform"/>. (These properties are automatically updated by
    /// the <see cref="UIRenderer"/>.)
    /// </para>
    /// <para>
    /// Render callbacks may cache information in <see cref="UIControl.RendererInfo"/>.
    /// </para>
    /// </remarks>
    ...
    private void RenderTextBlock(UIControl control, UIRenderContext context)
    {
      ...
      {
        var contentBounds = GetContentBoundsRounded(textBlock);
        ...
        if (textBlock.VisualClip)
        {
          ...
          var scissorRectangle = context.RenderTransform.Transform(contentBounds).ToRectangle(true);
          GraphicsDevice.ScissorRectangle = Rectangle.Intersect(scissorRectangle, originalScissorRectangle);
    ...
    private void RenderSlider(UIControl control, UIRenderContext context)
    {
      ...
      {
        var indicatorBounds = GetContentBoundsRounded(slider);
    ...
    private void RenderProgressBar(UIControl control, UIRenderContext context)
    {
      ...
      {
        var indicatorBounds = GetContentBoundsRounded(bar);
    ...
    private void RenderConsole(UIControl control, UIRenderContext context)
    {
      ...
      if (console != null)
      {
        var contentBounds = GetContentBoundsRounded(console);

        // Draw all visual lines. A fixed-width font is assumed and the text was already prepared -
        // no clipping necessary.
        var font = GetFont(console.Font);
        var foreground = GetForeground(control, GetState(context), context.Opacity);
            ...
            && IsCaretVisible(new Vector2F(console.VisualCaretX, console.VisualCaretY)))
        {
          ...
          var caretRectangle = new RectangleF(
    ...
    private void RenderTextBox(UIControl control, UIRenderContext context)
    {
      ...
      // TODO: TextBox should determine if clipping is nessary and set a VisualClip flag.
      ...
      if (textBox != null)
      {
        var contentBounds = GetContentBoundsRounded(textBox);
        var originalScissorRectangle = GraphicsDevice.ScissorRectangle;
    ...
    public void RenderImages(UIControl control, UIRenderContext context, bool drawOverlays)
    {
      ...
      var numberOfImages = state.Images.Count;
      if (numberOfImages == 0)
        return;

      var bounds = GetActualBoundsRounded(control);
    ...
    public void RenderImage(RectangleF bounds, ThemeImage image, float opacity, RenderTransform transform)
    {
      ...
      var source = image.SourceRectangle;
      var margin = image.Margin;
      bounds.X += margin.X;
      bounds.Y += margin.Y;
      bounds.Width = bounds.Width - margin.X - margin.Z;
      bounds.Height = bounds.Height - margin.Y - margin.W;

      // Get tint color and premultiply alpha.
      var colorVector = image.Color.ToVector4();
      colorVector.W *= opacity;
      var color = Color.FromNonPremultiplied(colorVector);
    ...
    private void RenderTileX(Texture2D texture, RectangleF area, RectangleF destination, Rectangle source, RenderTransform transform, Color color)
    {
      ...
      var clippedDestination = destination;
      var clippedSource = source;
