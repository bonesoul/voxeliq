...
namespace DigitalRune.Game.UI.Rendering
{
  ...
  /// renderers can use different methods. The renderer has a <see cref="SpriteBatch"/> that is
  /// used to render the images. <see cref="BeginBatch"/> can be called to start a batch with 
  /// the default settings of the renderer. <see cref="BeginBatch"/> is normally automatically
  /// called when a <see cref="UIScreen"/> is rendered and the batch is finished with
  /// <see cref="EndBatch"/> when the whole screen was rendered. This way all controls are 
  /// rendered as a single batch. If a control must be rendered with different render states
  /// or a different sprite batch, <see cref="EndBatch"/> must be called to flush the current
  /// batch.
  /// </para>
  /// </remarks>
  public interface IUIRenderer 
  {
    ...
    /// <summary>
    /// Calls <strong>SpriteBatch.Begin</strong> of the <see cref="SpriteBatch"/> with default 
    /// settings. 
    /// </summary>
    /// <remarks>
    ...
    /// <summary>
    /// Calls <strong>SpriteBatch.End</strong> of the <see cref="SpriteBatch"/> to commit the 
    /// current batch.
    /// </summary>
    void EndBatch();
  }
