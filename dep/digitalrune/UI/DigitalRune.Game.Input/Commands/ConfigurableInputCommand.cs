...
namespace DigitalRune.Game.Input
{
  ...
  public class ConfigurableInputCommand : IInputCommand
  {
    ...
    /// <remarks>
    /// <para>
    /// A <see cref="Sensitivity"/> of 1 creates a linear response curve (default). 
    /// A value of 1/2 creates a quadratic response curve. A value of 1/3 creates a cubic
    /// response curve. Etc.
    /// </para>
    /// <para>
    /// The <see cref="Sensitivity"/> does not change the minimal and maximal values of the
    /// <see cref="Value"/>. But they change how the <see cref="Value"/> changes in response
    /// to analog input. If a thumb stick is pressed half way and the <see cref="Sensitivity"/>
    /// is 1, the output <see cref="Value"/> is 0.5. If the <see cref="Sensitivity"/> is less
    /// than 1, then the <see cref="Value"/> would be less than 0.5f. If the 
    /// <see cref="Sensitivity"/> is greater than 1, then the <see cref="Value"/> would be greater
    /// than 0.5f. In other words: If the <see cref="Sensitivity"/> is high, then a small thumb 
    /// stick movement has a larger reaction.
    /// </para>
    /// </remarks>
